...
steps = [
    step(
        ...
        SELECT * FROM my_notifications_ctx;
        ...
        CREATE OR REPLACE images_default_albums AS
        SELECT
            i.image_id,
        ...
        WHERE
            ie.event_id = cur_event_profile_uuid('event_id')
        ...
        SELECT ar.*
        FROM access_requests ar
        WHERE ar.applicant_profile_id = cur_profile_uuid('profile_id')
        AND ar.event_id = cur_event_profile_uuid('event_id');        
        ...
        WHERE cur_profile_int('hierarchy_rank') > 0
        AND arg.event_id = cur_event_profile_uuid('event_id');
        ...
        WHERE EXISTS (
            SELECT 1
            FROM access_requests_groups_ctx ard
            WHERE ard.access_request_id = ar.access_request_id
        );
        
        CREATE OR REPLACE VIEW access_requests_ext AS
        SELECT ard.*
        FROM access_requests_ctx arc
        INNRT JOIN access_requests_details ard ON arc.access_request_id = ard.access_request_id;
        ...
        FROM profiles p
        LEFT JOIN my_notifications_ctx mn ON p.profile_id = mn.profile_id
