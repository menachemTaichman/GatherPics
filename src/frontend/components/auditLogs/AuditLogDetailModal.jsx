...
export default function AuditLogDetailModal({ 
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-[60] flex items-center justify-center p-4 pointer-events-none" dir={isRTL ? 'rtl' : 'ltr'}>
      <motion.div
        ref={modalRef}
        tabIndex={-1}
      ...
      >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-semibold text-gray-900 flex items-center gap-2">
                <span>{t('auditLogDetail.auditLog')} #{auditLog.audit_log_id}</span>
              ...
              <p className="text-sm text-gray-500">
                ...
                {totalLogs > 1 && (
                  <span className="ml-2">• {currentIndex + 1} {t('auditLogDetail.of')} {totalLogs}</span>
                )}
              </p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
            <h3 className="text-sm font-semibold text-gray-700 mb-3">{t('auditLogDetail.action')}</h3>
            <p className="text-lg font-medium text-gray-900">{formatAction(auditLog.action)}</p>
          </div>

          {/* Actor Information */}
          <div className="bg-gray-50 rounded-lg p-4">
            <h3 className="text-sm font-semibold text-gray-700 mb-3">{t('auditLogDetail.actorInformation')}</h3>
            <div className="space-y-2">
              {auditLog.actor_profile_label && (
                <div className="flex items-center gap-2 text-sm">
                  <User className="w-4 h-4 text-gray-400" />
                  <span className="text-gray-600">{t('auditLogDetail.profile')}</span>
              ...
              {auditLog.actor_profile_id && (
                <div className="flex items-center gap-2 text-sm">
                  <User className="w-4 h-4 text-gray-400" />
                  <span className="text-gray-600">{t('auditLogDetail.profileId')}</span>
              ...
              {auditLog.ip_address && (
                <div className="flex items-center gap-2 text-sm">
                  ...
                  <span className="text-gray-600">{t('auditLogDetail.ipAddress')}</span>
          ...
          {auditLog.details && Object.keys(auditLog.details).length > 0 && (
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
              ...
              >
                <span>{t('auditLogDetail.detailsJson')}</span>
                {showDetails ? (
