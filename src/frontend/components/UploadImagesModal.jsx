...
export default function UploadImagesModal({ 
  ...
  const validateAndAddFiles = useCallback((files) => {
    ...
    // Check count limit
    if (uploadLimits && uploadLimits.available_images_count !== -1) {
      const totalFiles = selectedFiles.length + jpgFiles.length;
      if (totalFiles > uploadLimits.available_images_count) {
        showToast(
          `Cannot add ${jpgFiles.length} files. You can only upload ${uploadLimits.available_images_count} more image(s). Limit: ${uploadLimits.images_count_limit}`, 
          'error'
        );
        return;
      }
  ...
  }, [selectedFiles, uploadLimits, showToast]);
