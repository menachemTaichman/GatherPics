...
@app.route("/api/groups/auto-merge", methods=["POST"])
def auto_merge_duplicate_groups():
    """Automatically merge groups that contain duplicate faces"""
    try:
        data = request.get_json() or {}
        merge_strategy = data.get('mergeStrategy', 'smallest_id')
        ...
        if not duplicates:
            return jsonify({
                "success": True,
                "message": "No groups needed merging"
            })
        ...
        for group_set, face_ids in group_sets.items():
            ...
            if merge_strategy == 'smallest_id':
                target_group_id = min(group_set)
                groups_to_merge = [g for g in group_set if g != target_group_id]
            elif merge_strategy == 'largest_count':
                group_counts = {}
                for group_id in group_set:
                    group = next((g for g in groups if g['groupID'] == group_id), None)
                    group_counts[group_id] = len(group['faceIDs']) if group else 0
                target_group_id = max(group_counts.items(), key=lambda x: x[1])[0]
                groups_to_merge = [g for g in group_set if g != target_group_id]
            ...
            for group_to_merge in groups_to_merge:
                # Find the groups
                target_group = next((g for g in groups if g['groupID'] == target_group_id), None)
                group_to_merge_obj = next((g for g in groups if g['groupID'] == group_to_merge), None)
                
                if target_group and group_to_merge_obj:
                    # Merge face IDs from group_to_merge into target_group
                    merged_face_ids = list(set(target_group['faceIDs'] + group_to_merge_obj['faceIDs']))
                    target_group['faceIDs'] = merged_face_ids
                    
                    # Update all faces that belonged to group_to_merge to now belong to target_group_id
                    for face in faces:
                        if face['groupID'] == group_to_merge:
                            face['groupID'] = target_group_id
                    
                    # Remove group_to_merge from groups list
                    groups = [g for g in groups if g['groupID'] != group_to_merge]
                    
                    merges_performed.append((target_group_id, group_to_merge))
        ...
        if merges_performed:
            ...
            if save_groups(groups):
                ...
                return jsonify({
                    "success": True,
                    "message": f"Successfully performed {len(merges_performed)} merges",
                    "mergesPerformed": merges_performed
                })
            else:
                return jsonify({"error": "Failed to save merged groups"}), 500
        else:
            return jsonify({
                "success": True,
                "message": "No groups needed merging"
        ...
        print(f"Error auto-merging groups: {e}")
        import traceback
        traceback.print_exc()
        return jsonify({"error": f"Failed to auto-merge groups: {str(e)}"}), 500

@app.route("/api/groups/duplicates", methods=["GET"])
def get_duplicate_faces():
    """Get information about faces that appear in multiple groups"""
    try:
        # Use the existing data loading functions instead of FaceClusterAWS
        ...
        for face_id, group_ids in duplicates.items():
            ...
            for group_id in group_ids:
                face_info = next((f for f in faces_data if f['faceID'] == face_id and f['groupID'] == group_id), None)
                if face_info:
                    face_details.append({
                        'group_id': group_id,
                        'image_id': face_info['imageID'],
        ...
        return jsonify({
            ...
            "count": len(duplicate_info)
        })
        
    except Exception as e:
        print(f"Error getting duplicate faces: {e}")
        import traceback
        traceback.print_exc()
        return jsonify({"error": f"Failed to get duplicate faces: {str(e)}"}), 500
